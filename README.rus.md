...
- **Управление GPU-слоями** - параметр `gpulayer` на уровне запроса для частичной разгрузки на GPU (0 = только CPU, N = N слоёв на GPU, не указан = авто)
- **Размер контекста на уровне запроса** - необязательный параметр `contextSize` задаёт размер контекстного окна LLM в токенах; при изменении между запросами контекст автоматически пересоздаётся (отсутствие параметра — отдельное состояние, означающее «использовать дефолт модели»)
- **Эндпоинт эмбеддингов** - `POST /embedding` генерирует векторные представления текста с использованием любой GGUF embedding-модели; использует общую очередь и принцип «одна модель в памяти» совместно с `/prompt`
- **Серверные инструменты** — LLM может вызывать JS-функции, описанные в файлах `*.tool.txt`; код инструмента имеет доступ к `LIB` (fs, path, math, коннекторы к PostgreSQL / MSSQL / Oracle); секреты подключения хранятся в файлах `*.toolenv.txt` и никогда не передаются модели
- **Простой REST API** - легкая интеграция с любым языком программирования или инструментом
- **Веб-интерфейс** - интерактивный чат для тестирования и экспериментов, с отображением числа слоёв и режима рассуждений модели
...
- [Серверные инструменты](#серверные-инструменты)
...
```jsonc
"toolDir": "/путь/к/инструментам"  // Директория с файлами *.tool.txt
```

Сервис автоматически обнаруживает все файлы `*.tool.txt` в этой директории. Каждый файл описывает один серверный инструмент, который LLM может вызвать. Формат файлов описан в разделе [Серверные инструменты](#серверные-инструменты).

### Директория переменных окружения инструментов
```jsonc
"toolEnvDir": "/путь/к/env-переменным"  // Директория с файлами *.toolenv.txt
```

Содержит файлы `*.toolenv.txt` — простые JSON-объекты, на которые ссылаются в коде инструментов через синтаксис `{{имя-файла}}` (без расширения `.toolenv.txt`). Используется для хранения секретов (учётные данные БД, API-ключи), которые никогда не передаются модели.
...
Серверные инструменты позволяют LLM вызывать JavaScript-функции во время генерации. Модель самостоятельно решает, когда и как их использовать; сервис выполняет код и возвращает результат обратно в диалог.

### Формат файла инструмента (`*.tool.txt`)

Размещайте файлы в директории, настроенной как `toolDir`. Каждый файл состоит из двух секций:
...
Имя инструмента в запросах — имя файла (например, `my-query.tool.txt` → имя `my-query.tool.txt`).

### Доступные объекты LIB в коде инструмента
...
### Переменные окружения (`*.toolenv.txt`)

Размещайте JSON-файлы в `toolEnvDir`. Ссылайтесь на них в коде инструмента через `{{имя-файла}}` (без `.toolenv.txt`):
...
Переменные окружения разрешаются в момент вызова инструмента LLM — не при запуске сервиса и не при сборке списка инструментов. Если referenced-переменная отсутствует в `toolEnvDir`, промпт возвращает ошибку 500 с понятным сообщением: `tool "my-query.tool.txt": env var "{{my-pg-conn}}" not found in toolEnvDir`.

### Использование инструментов в запросе
...
curl -X POST http://localhost:19777/prompt \
  ...
  -d '{
    ...
    "message": { "user": "Сколько таблиц в схеме public?" },
    "durationMsec": 60000,
    "toolServer": ["my-query.tool.txt"]
  }'
```

В ответе появляется `result.usedTools` — список имён инструментов в порядке их вызова:
...
```json
{
  ...
  "result": {
    ...
    "data": "В схеме public находится 42 таблицы."
  }
}
```

Готовые примеры смотрите в [examples/tool/](examples/tool/).
...
```typescript
{
  ...
  gpulayer?: number;          // Слои на GPU (0 = только CPU, N = N слоёв на GPU, не указан = авто)
  contextSize?: number;       // Размер контекстного окна в токенах (мин. 512); не указан = дефолт модели; изменение пересоздаёт контекст
  message: {
...
```typescript
{
  ...
  result: {
    ...
    usedTools?: string[];     // Имена инструментов в порядке вызовов (присутствует только при toolServer)
...
#### `POST /embedding`
Генерация векторного представления (эмбеддинга) для текстовой строки с помощью локальной GGUF embedding-модели.

**Тело запроса**:
```typescript
{
  model: string;       // Имя модели (имя файла с .gguf), например "bge-m3-Q8_0.gguf"
  gpulayer?: number;   // Слои на GPU (0 = только CPU, N = N слоёв на GPU, не указан = авто)
  message: string;     // Текст для преобразования в вектор
}
```
...
```typescript
{
  ...
  result: {
    ...
    data: number[];       // Вектор эмбеддинга (значения float32)
  };
}
```
...
> **Примечание:** Эндпоинт эмбеддингов использует общую очередь запросов и принцип «одна модель в памяти» совместно с `/prompt`. Если в памяти загружена чат-модель, она будет выгружена перед загрузкой embedding-модели, и наоборот. Для лучших результатов используйте специализированные embedding-модели: `bge-m3`, `nomic-embed-text`, `all-minilm`.
...
#### `GET /state/models`
Список всех доступных моделей в директории моделей. Каждая запись включает `layerCount` (число трансформерных слоёв, считанное из заголовка GGUF, если доступно) и `isReasoning` (есть ли у модели режим рассуждений/thinking на основе её chat template).
...
- **/chat** - Интерактивный чат-интерфейс для тестирования промптов; включает поле `gpu layer` для управления разгрузкой на GPU на уровне запроса (сохраняется в localStorage)
- **/doc** - Swagger/OpenAPI API документация
- **/stat** - Панель статистики; карточка **available models** показывает общее количество моделей (кликабельно — открывает список) и суммарный размер на диске; карточка **loaded model** показывает имя, размер, размер контекста (или `default`), gpu layer (или `auto`) и время загрузки текущей модели
- **/log/core** - Логи основных операций сервиса
- **/log/chat** - Логи запросов/ответов для промптов

Имена моделей в выпадающих списках отображаются с дополнительной информацией из заголовка GGUF: `model-name.gguf (498 MB, THINKING, 24 LAYER(s))`. `THINKING` показывается только для моделей с режимом рассуждений (например, DeepSeek-R1, Qwen3).
