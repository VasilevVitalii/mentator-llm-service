...
async function onHelp() {
	console.log(
		[
			`mentator-llm-service, version ${VERSION}`,
			`Service for get prompts and generate answer in JSON format`,
			``,
			`Usage modes:`,
			``,
			...
			``,
			`For more details, see https://github.com/VasilevVitalii/mentator-llm-service`,
		].join('\n'),
	)
}
